...
CASE nError=1160 and not this.lQuiet
	* Out of disk space
	lnUserChoiceMESSAGEBOX( NO_DISK_SPACE_LOC, RETRY_CANCEL+ICON_EXCLAMATION, TITLE_TEXT_LOC )
...
FUNCTION RemotizeViews
	...
	* get messed up
	...
	SELECT LOWER( TblName ), LOWER( RmtTblName ), ISNULL( TblName ), 1/LEN( ALLTRIM( TblName )) AS foo ;
	...
	FOR I=1 TO ALEN( aViews, 1 )
		...
		* Check which ( if any ) tables were upsized
		...
		FOR ii=1 TO ALEN( aTableNames, 1 )
			IF LOWER( RTRIM( aTableNames[ii, 1] )) + ", " $ lcTablesInView
				* set flag
				llTableUpsized=.T.
	...
	*
	...
	FOR I=1 TO ALEN( aTableNames, 1 )
		...
		* If not but unique index is available, use that
		IF EMPTY( aPkey ) THEN
			SELECT RmtExpr ;
			  FROM ( lcEnumIndexesTbl ) ;
			 WHERE RTRIM( IndexName )==RTRIM( aTableNames[i, 1] ) AND ;
			       RmtType="UNIQUE" ;
			  INTO ARRAY aPkey
		ENDIF
